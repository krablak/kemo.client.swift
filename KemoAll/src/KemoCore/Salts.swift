...
public struct Salts {
	 ...
	 */
	public static func saltSessionPath(_ key: [UInt8]) -> [UInt8] {
	 ...
	 */
	public static func saltEncKey(_ key: [UInt8]) -> [UInt8] {
	...
	fileprivate static func salt(_ key: [UInt8], saltingData: SaltingData, keyBytesFilterFn: (Int,UInt8) -> Bool) -> [UInt8] {
		if key.count == 0 {
			...
			for i in 0...base64Bytes.count - 1 {
				if keyBytesFilterFn(i, base64Bytes[i]) {
